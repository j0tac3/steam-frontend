...
export class BibliotecaComponent implements OnInit {
    ...
    // 1. Señales para los filtros y ordenación
    filtroTexto = signal('');
    ...
    bibliotecaFiltrada = computed(() => {
      ...
      let lista = this.myLibrary().filter(juego => {
        // Normalizamos el nombre (Steam usa 'name', Laravel usa 'title')
      ...
      return lista.sort((a, b) => {
        if (orden === 'nombre') {
          ...
          // Orden por 'reciente' (ID más alto primero)
    ...
    verDetalles(game: any) {
      // 1. Activamos la carga y seteamos el juego base INMEDIATAMENTE
      // Esto hace que el @if de Angular sea 'true' al instante y el modal se abra
      ...
      if (esIgdb) {
        this.steamService.getIgdbDetails(gameId).subscribe({
          next: (data) => {
            // 2. Actualizamos con los detalles completos cuando llegan
